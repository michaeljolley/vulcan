...
fetch('/socketio')
  ...
  .then(payload => {
    ...
    socket.on('onCreditRoll', streamEventArg => {
      ...
      if (streamEventArg && streamEventArg.stream) {
        ...
        /* Moderators */
        if (stream.moderators && stream.moderators.length > 0) {
        ...
        if (stream.subscribers && stream.subscribers.length > 0) {
          ...
          const subscribers = stream.subscribers
            .map(m => m.user)
            .filter(n => {
              return tempSubs.indexOf(n.id) === -1 && tempSubs.push(n.id);
            });

          subscribers.forEach(sub => {
            marquee.append(
              addParagraph(
                sub.display_name || sub.login,
                false,
        ...
        /* Cheerers */
        if (stream.cheers && stream.cheers.length > 0) {
          ...
          const cheerers = stream.cheers
            .map(m => m.user)
            .filter(n => {
              return tempCheers.indexOf(n.id) === -1 && tempCheers.push(n.id);
            });

          cheerers.forEach(cheer => {
            marquee.append(
              addParagraph(
                cheer.display_name || cheer.login,
                false,
                cheer.profile_image_url
              )
            );
          });
        }
        ...
        /* Raiders */
        if (stream.raiders && stream.raiders.length > 0) {
          ...
          const raiders = stream.raiders
            .map(m => m.user)
            .filter(n => {
              return tempRaiders.indexOf(n.id) === -1 && tempRaiders.push(n.id);
            });

          raiders.forEach(raider => {
            marquee.append(
              addParagraph(
                raider.display_name || raider.login,
                false,
                raider.profile_image_url
              )
            );
          });
        }
        ...
        /* Followers */
        if (stream.followers && stream.followers.length > 0) {
          ...
          const tempFollows = [];
          const followers = stream.followers.filter(n => {
            return tempFollows.indexOf(n.id) === -1 && tempFollows.push(n.id);
          });

          followers.forEach(follow => {
            marquee.append(
              addParagraph(
                follow.display_name || follow.login,
                false,
        ...
        if (
          ...
          contributors = contributors.filter(n => {
            return (
          ...
          contributors.forEach(contributor => {
            marquee.append(
              addParagraph(
                contributor.display_name || contributor.login,
                false,
